...
RUFF ?= .venv/bin/ruff
MYPY ?= .venv/bin/mypy
STRATA ?= .venv/bin/strata

help: ## Display available Makefile targets
	@echo "Strata Development & Administration Makefile"
	@echo "============================================="
...
test-cov: ## Execute pytest test suite with code coverage report
	$(PYTEST) --cov=strata --cov-report=term-missing tests/
...
docker-build: ## Build production Docker container image (strata:latest)
	docker build -t strata:latest .

docker-run: ## Run production Docker container binding port 8000
	docker run -d --name strata_engine -p 8000:8000 strata:latest

server: ## Launch local FastAPI REST API server
	$(STRATA) server start --host 127.0.0.1 --port 8000

doctor: ## Run Strata environment diagnostics check
	$(STRATA) doctor
