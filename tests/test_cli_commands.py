...
""")
    ...
    res_exp = cli_runner.invoke(cli, ["dag", "export", "cli_lifecycle_dag", "-o", str(export_file), "--db-url", db_url])
...
""")
    ...
    # 2. Run list
    res_list = cli_runner.invoke(cli, ["run", "list", "--dag-id", "cli_run_target", "--db-url", db_url])
    assert res_list.exit_code == 0
...
""")
    ...
    res_start = cli_runner.invoke(
        cli,
        ["run", "start", str(dag_file), "--run-id", "test_run_123", "--db-url", db_url, "--format", "json"],
    )
    assert res_start.exit_code == 0

    # 1. Check run status by run ID
    res_status = cli_runner.invoke(cli, ["run", "status", "test_run_123", "--db-url", db_url, "--format", "json"])
    assert res_status.exit_code == 0
    data_status = json.loads(res_status.output)
    assert data_status["run_id"] == "test_run_123"
    assert data_status["state"] == "completed"

    # 2. Check step output details
    res_step = cli_runner.invoke(cli, ["run", "step", "test_run_123", "step_out", "--db-url", db_url])
...
""")
    ...
    cli_runner.invoke(cli, ["run", "start", str(dag_file), "--run-id", "completed_run_1", "--db-url", db_url])

    # 1. Retry completed run (expect error)
    res_retry_completed = cli_runner.invoke(cli, ["run", "retry", "completed_run_1", "--db-url", db_url])
    ...
    res_retry_missing = cli_runner.invoke(cli, ["run", "retry", "missing_run_999", "--db-url", db_url])
...
def test_cli_server_info_and_metrics_with_mock(cli_runner: CliRunner) -> None:
    ...
    with patch("httpx.get", return_value=mock_resp):
        ...
        res_metrics = cli_runner.invoke(cli, ["server", "metrics", "--url", "http://127.0.0.1:8000"])
...
def test_cli_server_start_command_dry_run(cli_runner: CliRunner) -> None:
    ...
    with patch("uvicorn.run") as mock_uvicorn:
        res_start = cli_runner.invoke(cli, ["server", "start", "--port", "9000", "--host", "0.0.0.0"])
        assert res_start.exit_code == 0
