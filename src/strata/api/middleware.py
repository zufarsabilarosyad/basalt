...
from strata.api.schemas import ErrorDetail, ErrorResponse, create_error_response
from strata.core.dag.exceptions import StrataError
from strata.utils.crypto import generate_uuid
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        response.headers["X-Strata-Engine-Version"] = "1.0.0"
        return response
...
def setup_exception_handlers(app: FastAPI) -> None:
    ...
    async def strata_error_handler(request: Request, exc: StrataError) -> JSONResponse:
        req_id = getattr(request.state, "request_id", "unknown")
        logger.warning(f"[{req_id}] StrataError caught: [{exc.code}] {exc.message}")
