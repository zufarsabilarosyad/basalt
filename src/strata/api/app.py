...
from strata.api.middleware import (
...
from strata.api.router_dags import router as router_dags
from strata.api.router_runs import router as router_runs
from strata.api.router_triggers import router as router_triggers
from strata.api.schemas import HealthResponse, SystemInfoResponse
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    """FastAPI async lifespan context manager initializing and shutting down StrataEngine."""
    logger.info("Initializing StrataEngine facade for FastAPI application lifespan...")
    engine = get_engine()
    await engine.start()

    yield

    logger.info("Shutting down StrataEngine facade from FastAPI application lifespan...")
    await engine.stop()
...
def create_app(config: EngineConfig | None = None) -> FastAPI:
    ...
    """
    app = FastAPI(
        title="Strata DAG Execution Engine REST API",
    ...
    async def root_info() -> dict:
        return {
            "name": "Strata DAG Execution Engine API",
            "version": "1.0.0",
            "documentation": "/docs",
            "health": "/health",
            "metrics": "/metrics",
