"""Complete Async DAG Workflow Lifecycle Runner Subsystem Module for Strata Engine.
...
from strata.core.dag.ast import DAGSpec, StepSpec
...
from strata.core.engine.evaluator import ExpressionEvaluator
from strata.core.engine.hooks import HookRegistry, LifecycleEvent, get_hook_registry
from strata.core.engine.state_machine import StateMachine, StepState, WorkflowState
    ...
    async def run_async(
        ...
        try:
            ...
            for level_idx, level_steps in enumerate(execution_levels):
                ...
                # Trigger STEP_START hook for all runnable steps in level
                for step in runnable_steps:
                    await self.hook_registry.trigger(
                        LifecycleEvent.STEP_START, context, {"step_id": step.id}
                    )

                # Execute level steps in parallel via WorkerPool
                level_results = await self.worker_pool.execute_steps_parallel(
                    runnable_steps, context
                )
                ...
                for step in runnable_steps:
                    ...
                    if state == StepState.COMPLETED:
                        ...
                        await self.hook_registry.trigger(
                            ...
                            context,
                            {"step_id": step.id, "error": step_err},
                        )
