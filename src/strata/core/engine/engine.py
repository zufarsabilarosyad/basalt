"""Top-Level StrataEngine Facade Subsystem Module.

Provides StrataEngine class unifying YAML/JSON DAG parsing, topological sorting validation,
...
from strata.core.engine.runner import WorkflowRunner, WorkflowRunResult
from strata.core.engine.state_machine import WorkflowState
from strata.core.executors.pool import WorkerPool
from strata.core.storage.memory import MemoryStorageEngine
from strata.core.triggers.base import TriggerEvent
from strata.core.triggers.dispatcher import TriggerDispatcher
from strata.core.triggers.webhook import WebhookTrigger
from strata.storage.database import get_db_manager
from strata.storage.repository import StrataRepository
from strata.utils.crypto import generate_run_id
...
class EngineConfig(BaseModel):
    """Configuration options for StrataEngine initialization."""

    db_url: str = Field(
        default="sqlite+aiosqlite:///strata.db",
...
class StrataEngine:
    """Master facade API unifying Strata workflow engine subsystems."""
    ...
    def __init__(self, config: EngineConfig | None = None) -> None:
        ...
        if self.config.use_memory_storage:
            ...
            self.repository = StrataRepository(db_manager=db_mgr)
    ...
    async def initialize(self) -> None:
        ...
        logger.info("StrataEngine subsystems initialized successfully")
    ...
    async def start(self) -> None:
        ...
        self._running = True
        logger.info("StrataEngine facade started")
    ...
    async def stop(self) -> None:
        ...
        self._running = False
        logger.info("StrataEngine facade stopped")

    async def __aenter__(self) -> "StrataEngine":
        await self.start()
        return self
    ...
    async def _handle_trigger_event(self, event: TriggerEvent) -> None:
        ...
        logger.info(
            f"StrataEngine processing TriggerEvent '{event.event_id}' for DAG '{event.dag_id}'"
        )
        try:
...
_engine_singleton: StrataEngine | None = None


def get_engine(config: EngineConfig | None = None) -> StrataEngine:
    """Retrieve process-wide StrataEngine singleton instance."""
