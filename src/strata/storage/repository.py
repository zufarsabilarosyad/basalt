"""High-Level Async Database Repository Subsystem Module for Strata Engine.

Provides StrataRepository class encapsulating async CRUD operations for DAG workflows,
...
from strata.storage.database import DatabaseManager, get_db_manager
from strata.storage.models import DAGModel, DAGRunModel, DLQModel, StepRunModel, TriggerModel
from strata.storage.serializers import ModelSerializer, json_dumps
...
def get_repository(db_url: str = "sqlite+aiosqlite:///strata.db") -> StrataRepository:
    """Retrieve StrataRepository instance using specified database URL."""
    db_mgr = get_db_manager(db_url=db_url)
    return StrataRepository(db_manager=db_mgr)
