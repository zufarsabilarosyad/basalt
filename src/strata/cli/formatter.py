"""CLI Output Formatter Subsystem Module for Strata Engine.
...
class CLIFormatter:
    """Formatter class converting Strata objects into terminal ASCII tables or JSON."""
    ...
    def format_run_result(
        ...
        step_headers = ["Step ID", "Execution State", "Output Summary"]
        step_rows = []
        for step_id, state in result.step_states.items():
            ...
            step_rows.append([step_id, state.value.upper(), output_str])
