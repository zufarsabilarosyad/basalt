...
from strata.cli.formatter import CLIFormatter
from strata.utils.logger import get_logger

logger = get_logger(__name__)


@click.group(name="server", help="Launch and manage Strata FastAPI HTTP REST server.")
def server_group() -> None:
    """HTTP REST server administration command group."""
    pass


@server_group.command(name="start", help="Start the Strata REST API HTTP server daemon.")
...
def start_server_cmd(host: str, port: int, reload: bool, workers: int) -> None:
    """Start uvicorn ASGI web server serving Strata REST API."""
    click.secho(
        f"🚀 Launching Strata REST API server on http://{host}:{port} (workers={workers}, reload={reload})...",
        fg="green",
        bold=True,
    )
...
@server_group.command(name="status", help="Probe health status of a running Strata server.")
@click.option("--url", default="http://127.0.0.1:8000", help="Strata HTTP server base URL.")
def server_status_cmd(url: str) -> None:
    """Check health and ping response from a running Strata API server instance."""
